...
description: "Validate that a story file is implementation-ready. Checks for embedded feature spec requirements, ADR references, engine notes, clear acceptance criteria, and no open design questions. Produces READY / NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is this story ready', 'can I start on this story', 'is story X ready to implement'."
...
- `docs/architecture/tr-registry.yaml` — index all entries by `id`. Used to
  validate TR-IDs in stories and resolve requirement text. If the file does
  not exist, note it once; TR-ID checks will auto-pass for all stories
  (registry predates stories, so missing registry means stories are from
  before TR tracking was introduced).
...
- [ ] **Feature spec requirement referenced**: The story includes a
  `docs/requirements/` path and quotes or links a specific requirement,
  acceptance criterion, or rule from that feature spec — not just the spec
  filename. A link to the document without tracing to a specific requirement
  does not pass.
- [ ] **Requirement is self-contained**: The acceptance criteria in the story
  are understandable without opening the spec. A developer should not need to
...
- [ ] **Manifest version is current**: If the story has a `Manifest Version:` date
  ...
  - Auto-pass if either the story has no `Manifest Version:` field OR the manifest
    does not exist.
- [ ] **Engine notes present**: For any post-cutoff UE 5.8 API this story is
  likely to touch, implementation notes or a Context7 verification requirement
  are included. If the story clearly does not touch engine APIs (e.g., it is a
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns
  (paths containing `Content/`, `assets/`, or UE asset extensions `.uasset`,
  `.umap`, plus common formats `.png`, `.jpg`, `.svg`, `.wav`, `.ogg`, `.mp3`).
...
Resolve these before the sprint begins.
```
...
- If a story file does not exist at all: "This story file is missing entirely.
  Run `/create-epics [layer]` then `/create-stories [epic-slug]` to generate stories from the feature spec and ADR."
- If a story has no spec reference and the work appears small: "This story has
  no feature spec reference. If the change is small (under ~4 hours), draft a
  minimal feature spec under `docs/requirements/` first, then reference it in
  the story."
- If a story's scope has grown beyond its original sizing: "This story appears
  to have expanded in scope. Consider splitting it into two stories before
