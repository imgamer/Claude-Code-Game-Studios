---
name: create-stories
description: "Break a single epic into implementable story files. Reads the epic, its feature spec, governing ADRs, and control manifest. Each story embeds its requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after /create-epics for each epic."
...
focused session, self-contained, and fully traceable to a feature spec
requirement and an ADR decision. Stories are what developers pick up. Epics are
...
- `production/epics/[epic-slug]/EPIC.md` — epic overview, governing ADRs, requirements table
- The epic's feature spec (`docs/requirements/[filename].md`) — read all
  sections, especially Acceptance Criteria, Formulas, and Edge Cases
- All governing ADRs listed in the epic — read the Decision, Implementation
  Guidelines, Engine Compatibility, and Engine Notes sections
- `docs/architecture/control-manifest.md` — extract rules for this epic's
  layer; note the Manifest Version date from the header
...
Report: "Loaded epic [name], feature spec [filename], [N] governing ADRs (all confirmed present), control manifest v[date]."
...
## 4. Decompose the Feature Spec into Stories

For each feature spec acceptance criterion:
...
- **Requirement**: which acceptance criterion(ia) does this satisfy?
...
Pass: the full story list with acceptance criteria, story types, and TR-IDs; the epic's feature spec acceptance criteria for reference.
...
**Engine**: Unreal Engine 5.8 | **Risk**: [LOW / MEDIUM / HIGH]
**Engine Notes**: [from ADR Engine Compatibility section — post-cutoff APIs, Context7 verification required]
...
*From feature spec `docs/requirements/[filename].md`, scoped to this story:*

- [ ] [criterion 1 — directly from spec]
- [ ] [criterion 2]
...
- Is this the last epic? If so, the next step is implementation.
...
5. **No invention** — acceptance criteria come from feature specs, implementation notes from ADRs, rules from the manifest
