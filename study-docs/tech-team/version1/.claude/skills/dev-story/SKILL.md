---
name: dev-story
description: "Read a story file and implement it. Loads the full context (story, feature spec requirement via TR-ID, ADR guidelines, control manifest), routes to the right programmer agent and UE 5.8 sub-specialist for the system, implements the code and test, and confirms each acceptance criterion. The core implementation skill — run after /story-readiness, before /code-review and /story-done."
argument-hint: "[story-path]"
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Bash, Task, AskUserQuestion, run_mcp
model: sonnet
---
...
all the context a programmer needs, routes to the correct specialist agent,
and drives implementation to completion — including writing the test.
...
**Output:** Source code + test file in the project's `Source/` and `tests/` directories.
...
|------|------|------------|
| TR registry | `docs/architecture/tr-registry.yaml` | **STOP** — "TR registry not found at `docs/architecture/tr-registry.yaml`. Run `/architecture-review` to bootstrap the registry from your feature specs and ADRs." |
...
- **TR-ID** — the requirement identifier (resolves to a feature spec requirement via the TR registry)
...
Read the current `requirement` text AND the `spec`/`source` field — the
feature spec is the source of truth for what the requirement is now. Do not
...
- The Engine Compatibility section (post-cutoff UE 5.8 APIs, Context7 verification results, known risks)
...
- `Engine:` value — pinned to Unreal Engine 5.8
- Naming conventions (UE C++ class prefixes, file names, Blueprint prefixes)
- Performance budgets (frame budget, memory ceiling)
- Forbidden patterns

**UE 5.8 Knowledge Authority rule** (from `CLAUDE.md`):
...
The story's ADR `Engine Compatibility` section already carries Context7-stamped
verification results from when the ADR was written. Use those as the starting
point. The spawned sub-agent (fresh Claude instance) MUST re-verify UE APIs by
calling Context7 itself — it cannot see this session's results.
...
### Primary agent routing table (UE 5.8)

| Story context | Primary agent | Secondary specialist (spawn when engine risk is HIGH or domain touches UE subsystem) |
|---|---|---|
| Foundation layer — any type | `engine-programmer` | `unreal-specialist` |
| Any layer — Type: UI | `ui-programmer` | `ue-umg-specialist` |
| Any layer — Type: Visual/Feel | `gameplay-programmer` | `unreal-specialist` |
| Core or Feature — gameplay mechanics (no GAS) | `gameplay-programmer` | `unreal-specialist` |
| Core or Feature — gameplay mechanics (touches GAS) | `gameplay-programmer` | `ue-gas-specialist` |
| Core or Feature — AI behaviour, pathfinding | `ai-programmer` | `unreal-specialist` |
| Core or Feature — networking, replication | `network-programmer` | `ue-replication-specialist` |
...
(or `.claude/docs/engine-specialists.md`) to get the configured primary
specialist (`unreal-specialist`). Spawn the **route-specific sub-specialist**
from the table above alongside the primary agent when:
- The story involves UE-specific APIs, patterns, or subsystems
- The ADR's `Knowledge Risk` is **HIGH** (post-cutoff API in use)
- The story's domain touches GAS, UMG/CommonUI, Blueprint architecture, or replication

**When engine risk is HIGH** (from the ADR's Engine Compatibility section):
always spawn the relevant UE sub-specialist, even for non-engine-facing stories.
High risk means the ADR records assumptions about post-cutoff UE 5.8 APIs that
need expert verification — and the sub-specialist must re-verify them via
Context7 (it is a fresh Claude instance and cannot see this session's results).
...
2. **Feature spec requirement**: look up TR-ID `[TR-XXX-NNN]` in `docs/architecture/tr-registry.yaml` — use the `requirement` field as source of truth and follow the `spec`/`source` field to read the full spec section if needed
...
8. **UE 5.8 Knowledge Authority instruction**: Any UE API, class, or subsystem referenced in the ADR or to be used in the implementation must be re-verified by the sub-agent itself via Context7 (`resolve-library-id(libraryName: "Unreal Engine")` → `query-docs` with the specific API name). Fall back to `WebSearch` against `docs.unrealengine.com/5.8/` if Context7 returns nothing. NEVER rely on training data alone for UE 5.4+ APIs. Stamp output: `API 验证:Context7 查询 [查询词] 于 [日期] — [确认/变更/未找到]`.
9. **Explicit instruction**: implement this story following the ADR guidelines, respect the manifest rules, stay within the story's Out of Scope boundaries. Write clean, doc-commented public APIs.

The agent should:
- Create or modify files in `Source/` (UE C++ root) following the ADR guidelines
...
- Re-verify any UE 5.8 API via Context7 before using it
...
- Any UE 5.8-specific risks the specialist flagged (with Context7 verification stamps)
- The Context7 API verification stamps produced by the sub-agent
...
**Files changed**:
- `Source/[path]` — created / modified ([brief description])
...
**UE 5.8 engine risks flagged**: [None] or [specialist finding with Context7 stamp]
...
Common blockers:
- Input file missing (story not found, feature spec absent) → redirect to the skill that creates it
...
- Context7 cannot resolve the Unreal Engine library → fall back to WebSearch, flag as degraded knowledge authority in the summary
...
- **UE API verification is mandatory** — every UE 5.8 API used in the
  implementation must be Context7-stamped by the sub-agent. Do not accept
  "I know this API from training data" as justification.
