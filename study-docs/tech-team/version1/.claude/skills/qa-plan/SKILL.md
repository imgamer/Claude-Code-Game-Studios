---
name: qa-plan
description: "Generate a QA test plan for a sprint or feature. Reads feature specs and story files, classifies stories by test type (Logic/Integration/Visual/UI), and produces a structured test plan covering automated tests required, manual test cases, smoke test scope, and playtest sign-off requirements. Run before sprint begins or when starting a major feature."
...
story. It reads all in-scope story files and their referenced feature specs,
classifies each story by test type, and produces a plan that tells developers
exactly what to automate, what to verify manually, what the smoke test scope
is, and when to bring in a playtester.
...
- **Engine notes** — any UE 5.8 API warnings, post-cutoff API notes, or
  Context7 verification stamps from the ADR
- **Feature spec reference** — the spec path(s) cited
...
- `docs/architecture/tr-registry.yaml` — to understand system priorities and
  which requirements are active. The `spec`/`source` field points to the
  authoritative feature spec for each TR-ID.
- For each unique feature spec referenced across all stories: read the
  **Acceptance Criteria**, **Formulas**, and **Edge Cases** sections. Do not
  load the full spec text. These three sections contain the testable
  requirements, the math to verify, and the boundary conditions that tests must
  cover. If an Edge Cases section is absent from the spec, note it per spec:
  "No Edge Cases section found — edge case coverage will be inferred from
...
If no feature spec is referenced in a story, note it as a gap but do not block
the plan. The story will be classified using acceptance criteria alone.
...
**What to test**:
- [Specific formula or rule from the feature spec Formulas section]
...
- [Any edge case explicitly called out in the spec Edge Cases section]

**UE 5.8 API verification**: If the story's ADR flagged post-cutoff UE APIs,
note which APIs need Context7-verified test stubs/mocks. Tests should mock UE
subsystems using the signatures confirmed via Context7 — not the training-data
assumption.

**Estimated test count**: ~[N] unit tests

[If no spec formula reference was found for this story, note:]
*No formula found in referenced spec — test cases must be derived from acceptance
criteria directly. Review the spec Formulas section before writing tests.*
...
**Who must sign off**: [lead-programmer / qa-lead / technical-director]
...
the [qa-lead / technical-director] before the story can be marked COMPLETE.
...
When generating content, use the actual story titles, spec formula text, and
...
- **Classify conservatively**: when a story is ambiguous between Logic and
  ...
  integration tests.
- **Do not invent test cases** beyond what acceptance criteria and spec formulas
  support. If a formula is absent from the spec, flag it rather than guessing.
