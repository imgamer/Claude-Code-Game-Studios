---
name: story-done
description: "End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for spec/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint."
...
This skill closes the loop between requirements and implementation. Run it at the end
of implementing any story. It ensures every acceptance criterion is verified
before the story is marked done, feature spec and ADR deviations are explicitly
...
- **Story name and ID**
- **Requirement TR-ID(s)** referenced (e.g., `TR-combat-001`)
...
- **Engine notes** — any engine-specific constraints noted (post-cutoff UE 5.8 APIs, Context7 verification status)
...
- `docs/architecture/tr-registry.yaml` — look up each TR-ID in the story.
  Read the *current* `requirement` text AND the `spec`/`source` field from the
  registry entry. This is the source of truth for what the feature spec required —
  do not use any requirement text that may be quoted inline in the story (it may be stale).
- The referenced feature spec section — just the acceptance criteria and key
  rules for the system, not the full document. Use this to cross-check the
...
- **No hardcoded strings check**: `Grep` for player-facing strings in `Source/`
...
|-----------|------|--------|
| AC-1: [criterion text] | tests/unit/combat/test_hitbox.cpp::test_hitbox_detects | COVERED |
...
1. **Feature spec rules check**: Using the current requirement text from
   `tr-registry.yaml` (looked up by the story's TR-ID, with the `spec` field
   pointing to the source feature spec), check that the implementation reflects
   what the spec actually requires now — not what it required when the story
   was written. `Grep` the implemented files for key function names, data
   structures, or class names mentioned in the current spec section.
...
6. **UE 5.8 API drift check**: If the ADR's Engine Compatibility section flagged
   any post-cutoff APIs, verify the implementation uses the Context7-verified
   signature (not the training-data assumption). If a Context7 verification
   stamp from the dev-story session shows "变更" (changed), check the
   implementation matches the verified 5.8 signature, not the old one.

For each deviation found, categorize:

- **BLOCKING** — implementation contradicts the spec or ADR (must fix before
  marking complete)
...
Pass: implementation file paths, story file path, relevant feature spec section, governing ADR.
...
### Deviations
[NONE] OR:
- BLOCKING: [description] — [spec/ADR reference]
...
```
Suggested commit:
git add [Source/ and tests/ files changed during implementation] [story-file-path]
...
2. `/qa-plan sprint` — refresh the QA plan if needed
3. `/gate-check` — advance to the next phase once QA approves (only if advancing a phase)

Do not run `/gate-check` until QA has signed off.
```
...
- If tech debt was logged: track it in `docs/tech-debt-register.md` to keep the register current
