...
技能如何跨 7 个开发阶段串联的可视化地图。
这些图展示了每个技能之前和之后运行什么，以及它们之间流动什么产物。
...
UX 规格在阶段 4（生产前）编写，在史诗编写之前，以便故事验收标准可以引用特定 UX 产物。
...
| 在技术设置中 | `/create-architecture` → `/architecture-decision` |
