...
---
name: soak-test
description: "为长时间游戏会话生成浸泡测试协议。定义在长时间游戏会话期间要观察、测量和记录的内容，以发现缓慢泄漏、疲劳效应以及仅在持续游戏后才会出现的边界情况。主要用于 Polish 和 Release 阶段。"
argument-hint: "[duration: 30m | 1h | 2h | 4h] [focus: memory | stability | balance | all]"
user-invocable: true
allowed-tools: Read, Glob, Grep, Write
model: sonnet
...
浸泡测试（也称为耐久测试）是为了特定观察目标而运行的长时间游戏会话。与冒烟检查（广泛的关键路径，
约 10 分钟）或单功能试玩（约 30 分钟）不同，浸泡测试运行 **30
分钟到数小时** 以发现：
...
- **性能漂移** — 随时间恶化的帧时间退化
- **状态累积缺陷** — 仅在某个机制重复 N 次后出现的问题
  （库存满、分数溢出、AI 状态损坏）
- **乐趣疲劳** — 在第一次会话中感觉良好但在长时间游戏后变得
  重复的机制
- **内容耗尽** — 玩家耗尽新内容的节点
...
**时长**（默认：`1h`）：
- `30m` — 短时浸泡；适合测试单个机制或场景
- `1h` — 标准浸泡；涵盖最常见的泄漏类别
- `2h` — 扩展浸泡；推荐用于首次完整 Polish 浸泡
- `4h` — 深度浸泡；对于具有长会话设计的游戏（RPG、模拟游戏）必需
...
- `.claude/docs/technical-preferences.md` — 引擎（用于引擎特定的内存
  监控指南）、性能预算（内存上限、目标 FPS）
- `design/gdd/game-concept.md` — 预期会话时长（用于与
  浸泡时长比较）、核心循环描述
- `production/playtests/` 中最近的文件 — 先前的试玩发现
  （以避免重新记录已知问题）
- `production/qa/qa-plan-*.md` 中最近的文件 — 当前冲刺测试覆盖
  （了解什么已被正式测试 vs 浸泡覆盖什么）
...
- Note: `Performance.get_monitor(Performance.MEMORY_STATIC)` returns bytes
  in Godot 4.6
**Godot 4：**
- 打开 Debugger → Monitors 标签页；跨检查点跟踪 `Memory → Static Memory` 和
  `Object Count → Objects`
- 记录：Static Memory (KB)、Object Count、Orphan Nodes 计数
- 警报阈值：前 15 分钟后内存增长 > T+0 的 20%
  （加载时有一些增长是预期的；持续增长表示泄漏）
- 注意：`Performance.get_monitor(Performance.MEMORY_STATIC)` 在
  Godot 4.6 中返回字节
...
- 警报阈值：GC Allocated 在 3 个以上检查点间单调增长
...
- 警报阈值：整个浸泡期间 Physical Memory Used 增长 > 50MB
...
在每个检查点收集主观观察：
- [ ] 核心机制仍感觉有回报（Y/N）
- [ ] 感知难度水平：[too easy / appropriate / too hard]
- [ ] 自上次检查点以来是否有 "我以前见过这个" 的时刻？（新颖内容耗尽）
- [ ] 自上次检查点以来是否有沮丧时刻？注明原因。
- [ ] 自上次检查点以来是否有巅峰投入时刻？注明原因。
...
- **此技能生成协议 — 由人工运行它** — 切勿尝试
  自动运行浸泡测试。观察需要人工观察者。
- **时长应匹配游戏的会话设计** — 5 分钟的游戏
  不需要 4 小时浸泡；城市建设游戏可能需要。使用判断，如果不清楚则询问。
- **首次浸泡应为 `all` 焦点** — 窄焦点（仅内存）用于
  特定修复后的回归浸泡，而非首次通过
- **写入前询问** — 在创建协议文件之前始终确认
